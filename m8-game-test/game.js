...
let touchShape = null;
let touchShapeIndex = -1;

/* 手指按住方块 */
piecesEl.addEventListener("touchstart", (e) => {
  const piece = e.target.closest(".piece");
  if (!piece) return;

  e.preventDefault();

  const allPieces = Array.from(piecesEl.children);
  touchShapeIndex = allPieces.indexOf(piece);
  touchShape = currentShapes[touchShapeIndex];
});
...
boardEl.addEventListener("touchend", (e) => {
  ...
  if (target && target.classList.contains("cell")) {
    ...
    if (canPlace(touchShape, x, y)) {
      placeShape(touchShape, x, y);

      // ✅ 基础计分
      const placedCount =
        touchShape.flat().filter(v => v === 1).length;
      score += placedCount;
